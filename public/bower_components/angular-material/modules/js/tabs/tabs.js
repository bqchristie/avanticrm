 ...
 *  The Tabs components consists of clickable tabs that are aligned horizontally side-by-side.
 *
...
function MdTabsController ($scope, $element, $window, $mdConstant, $mdTabInkRipple, $mdUtil,
  ...
  /**
   * Binds events used by the tabs components.
   */
  function bindEvents () {
  ...
  /**
   * Queues up a call to `handleWindowResize` when a resize occurs while the tabs components is
   * hidden.
   */
 ...
 *
 * <i><b>Note:</b> If you are using the Tabs components for page-level navigation, please take a look
 * at the <a ng-href="./api/directive/mdNavBar">NavBar components</a> instead as it can handle this
 * case a bit more natively.</i>
 *
 * **Tabs with internal views** are the traditional usages where each tab has associated view
 * content and the view switching is managed internally by the Tabs components.
 *
