...
function expansionPanelDirective() {
  ...
  function controller($scope, $element, $attrs, $window, $$rAF, $mdConstant, $mdUtil, $mdComponentRegistry, $timeout, $q, $animate, $parse) {
    ...
    function registerPanel() {
      ...
      // deregister if components was already registered
      if (typeof deregister === 'function') {
        deregister();
        deregister = undefined;
      }
      // remove components from group ctrl if components was already added
 ...
 *
 * @description
 * Expand and collapse Expansion Panel using its `md-components-id`
 *
 * @example
...
function expansionPanelService($mdComponentRegistry, $mdUtil, $log) {
  var errorMsg = "ExpansionPanel '{0}' is not available! Did you use md-components-id='{0}'?";
  var service = {
    find: findInstance,
...
function expansionPanelGroupService($mdComponentRegistry, $mdUtil, $mdExpansionPanel, $templateRequest, $rootScope, $compile, $controller, $q, $log) {
  var errorMsg = "ExpansionPanelGroup '{0}' is not available! Did you use md-components-id='{0}'?";
  var service = {
    find: findInstance,
    waitFor: waitForInstance
  };
  ...
  function createGroupInstance(instance) {
    ...
    function add(options, locals) {
      locals = locals || {};
      // assume if options is a string then they are calling a registered card by its components id
      ...
      getTemplate(options, function (template) {
        ...
        var componentId = options.componentId || element.attr('md-components-id') || '_panelComponentId_' + $mdUtil.nextUid();
        var panelPromise = $mdExpansionPanel().waitFor(componentId);
        element.attr('md-components-id', componentId);
