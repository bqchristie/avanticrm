 ...
 * @description Module for the datepicker components.
 */
...
(function() {
   ...
   *
   * @param {Date} ng-model The components's model. Should be a Date object.
   ...
   *
   * @description
   * `<md-calendar>` is a components that renders a calendar that can be used to select a date.
  ...
  /**
   * Controller for the mdCalendar components.
  ...
  CalendarCtrl.prototype.handleKeyEvent = function(event) {
    ...
    this.$scope.$apply(function() {
      // Capture escape and emit back up so that a wrapping components
      ...
      // Broadcast the action that any child routes should take.
...
(function() {
  ...
  /**
   * Controller for the calendar month components.
   * ngInject @constructor
   */
...
(function() {
  ...
  /** Private components, representing a list of years in the calendar. */
  function calendarDirective() {
    return {
      template:
        '<div class="md-calendar-scroll-mask">' +
...
(function() {
  ...
  /**
   * Private components, consumed by the md-calendar-year, which separates the DOM construction logic
...
(function() {
  ...
  angular.module('material.components.datepicker').factory('$$mdDateUtil', function() {
    ...
    function createDateAtMidnight(opt_value) {
     ...
     /**
      * Checks if a date is within a min and max range, ignoring the time components.
...
(function() {
   ...
   * @module material.components.datepicker
   *
   * @param {Date} ng-model The components's model. Expects either a JavaScript Date object or a value that can be parsed into one (e.g. a ISO 8601 string).
   ...
   *
   * @description
   * `<md-datepicker>` is a components used to select a single date.
   * For information on how to configure internationalization for the date picker,
   * see `$mdDateLocaleProvider`.
   *
   * This components supports [ngMessages](https://docs.angularjs.org/api/ngMessages/directive/ngMessages).
  ...
  function datePickerDirective($$mdSvgRegistry, $mdUtil, $mdAria, inputDirective) {
    return {
      template: function(tElement, tAttrs) {
        ...
        // interaction on the text input, and multiple tab stops for one components (picker)
        // may be confusing.
  ...
  DatePickerCtrl.prototype.openCalendarPane = function(event) {
    if (!this.isCalendarOpen && !this.isDisabled && !this.inputFocusedOnWindowBlur) {
      ...
      // rest of the components (input, etc) is in a different scrolling container, such as
