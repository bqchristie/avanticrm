...
function SidenavService($mdComponentRegistry, $mdUtil, $q, $log) {
  var errorMsg = "SideNav '{0}' is not available! Did you use md-components-id='{0}'?";
  var service = {
  ...
  function addLegacyAPI(service, handle) {
     ...
     * registered with the markup `md-components-id`
     */
    function findInstance(handle, shouldWait) {
      ...
      if (!instance && !shouldWait) {
        ...
        // The components has not registered itself... most like NOT yet created
    ...
    /**
     * Asynchronously wait for the components instantiation,
     * Deferred lookup of components instance using $components registry
     */
 ...
 * @description
 *
 * A Sidenav components that can be opened and closed programatically.
 *
 ...
 * @param {string=} md-components-id componentId to use with $mdSidenav service.
...
function SidenavDirective($mdMedia, $mdUtil, $mdConstant, $mdTheming, $mdInteraction, $animate,
  ...
  function postLink(scope, element, attr, sidenavCtrl) {
    ...
    element.addClass('_md');     // private md components indicator for styling
    $mdTheming(element);
...
function SidenavController($scope, $attrs, $mdComponentRegistry, $q, $interpolate) {
  ...
  // Watch and update the components, if the id has changed.
  if (hasDataBinding) {
    $attrs.$observe('mdComponentId', function(id) {
      if (id && id !== self.$$mdHandle) {
        self.destroy(); // `destroy` only deregisters the old components id so we can add the new one.
